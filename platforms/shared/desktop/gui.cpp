...
{
    GC_Color ret;
    ret.red = (u8)(color.x * 255.0f);
    ret.green = (u8)(color.y * 255.0f);
    ret.blue = (u8)(color.z * 255.0f);
    return ret;
}

static ImVec4 color_int_to_float(GC_Color color)
{
    return ImVec4(color.red / 255.0f, color.green / 255.0f, color.blue / 255.0f, 1.0f);
}
...
void gui_update_palette(void)
{
    if (config_video.palette == 2)
    {
        for (int i = 0; i < 16; i++)
            config_video.color[i] = color_float_to_int(gui_custom_palette[i]);
...
static void set_style(void)
{
    ...
    style.Colors[ImGuiCol_SliderGrabActive] = ImVec4(0.8764706f, 0.74705882f, 0.66078431f, 1.0f);
